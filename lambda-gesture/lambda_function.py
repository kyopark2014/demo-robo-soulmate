...
def generate_gesture_message(chat, img_base64, query):    
    messages = [
        SystemMessage(content="답변은 20자 이내의 한국어로 해주세요. <result> tag를 붙여주세요."),
        HumanMessage(
            content=[
                {
                    "type": "image_url",
...
def lambda_handler(event, context):
    ...
    start_time_for_gesture = time.time()
    ...
    end_time_for_gesture = time.time()
    time_for_gesture = end_time_for_gesture - start_time_for_gesture
